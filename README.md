...
Проект настроен с автоматическим деплоем через GitHub Actions. При каждом пуше в ветку `main` автоматически:
- Проверяется код (линтинг)
- Собирается проект
- Деплоится на хостинг (если настроены секреты)

### Настройка автоматического деплоя

#### Для Netlify:

1. Получите токены в Netlify:
   - Перейдите в [Netlify User Settings](https://app.netlify.com/user/applications#personal-access-tokens)
   - Создайте Personal Access Token
   - Скопируйте Site ID из настроек вашего сайта
...
1. Получите токены в Vercel:
   - Перейдите в [Vercel Settings](https://vercel.com/account/tokens)
   - Создайте токен
   - Найдите Org ID и Project ID в настройках проекта
...
**Примечание:** Если секреты не настроены, workflow все равно будет собирать проект и создавать артефакты, но деплой не будет выполняться.
